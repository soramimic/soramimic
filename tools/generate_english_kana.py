...
## 辞書キーの形式について (js/English.js を精査した結果)

`English()` の `englishWordToKana()` は次の処理をする:

    let upper = text.toUpperCase();
    if (upper in e2k) return e2k[upper];

つまりキーは「単語を大文字化しただけ」のもので、アポストロフィは
そのまま (`'`) 残る。`Apostrophe` オブジェクトによる `'` → `"APOSTROPHE"`
という文字列展開は、外部トークナイザにアポストロフィを渡す際に
分割されるのを防ぐための内部処理でしかなく、辞書引きの直前
(`AP.toSign`) で必ず `'` に戻されてから `englishWordToKana` に渡る。
したがって生成する辞書のキーは `WORD.upper()` (アポストロフィはそのまま)
でよい。実際、旧 `bep-eng.json` にも `"DON'T"` のような素のアポストロフィ
キーと `"DONAPOSTROPHET"` のような展開済みキーが重複して入っていたが、
後者は使われていない不要なエントリだったとみられる。

また `alphabetToKana()` は変換しきれずに残ったアルファベット1文字ずつを
同じ辞書で引く (例: 頭字語のスペルアウト)。CMUdict では `a`〜`z` の
1文字エントリはすべて「文字の読み方」(bee, cee, dee, ...) を主発音として
持つが、`a` だけは例外で主発音が冠詞の "a" (曖昧母音 AH0) になっており、
2番目の発音 `a(2)` が文字読み (EY1, "エイ") である。1文字語は
`alphabetToKana` 用の文字読みとして使われる方が実用上重要なため、
このスクリプトでは1文字語について `(2)` の異形発音があればそちらを
優先する (該当するのは現行の cmudict.dict では `a` のみ)。
...
3. このスクリプトを実行する (全語彙 約13万語、e2k の推論が1語あたり
   20〜40ms 程度かかるため、並列化していても数分〜十数分かかる。
   端末が使えない/タイムアウトする環境ではバックグラウンド実行し、
   `--log` で指定したログファイルで進捗を確認するとよい):
...
4. 生成された `data/english-kana.json` と `data/english-kana.LICENSE` を
   ...
   (手順1で都度取得する想定。念のためコミットしても問題ないライセンスだが、
   3.6MB程度あり生成物ではなく単なる素材のため同梱を避けている)。
