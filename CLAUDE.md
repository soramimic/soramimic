...
4. dev/preview向けの非ドラフト・同一repository PRは**CI全通過で automerge が自動マージ**し、それぞれ[dev.soramimic.pages.dev](https://dev.soramimic.pages.dev) / [preview.soramimic.pages.dev](https://preview.soramimic.pages.dev)へ自動デプロイする。fork、`no-automerge`、`emergency`は対象外。main向けPRは自動マージせず、開発者の明示承認後に手動マージする。mainは本番(soramimic.com)へ対応する。コミットメッセージは日本語
