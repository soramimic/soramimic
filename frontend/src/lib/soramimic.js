...
const SoramimiMaker = (kanaSimilarity, textAnalyzer)=>{
	...
	const getSimilarWord = (wordlist,target,kanaDist,length=1,variationCost=0) => {
		...
		for(let i in candidates){
			for(let w of wordlist[i]){
				...
				for(let c of candidates[i]){
					...
					let d = ld(c, w.pronunciation, kanaDist) + ((c.vcost||0)+(w.vcost||0))*variationCost;
					sim = Math.min(d, sim);
				}
	...
	function convert(tokens, getSimilarWordFunc, used_words, param={}, locks=null){
        ...
        const dp = (s,t) => {
			...
			for (let i = s; i<t; i++){
				...
				const similarWords = getSimilarWordFunc(subtarget);
				if(!similarWords){
					continue;
				}
	...
	function getCandidates(wordlist, target, parameter, length=30){
		const param = assignDefaultParameter(parameter);
		const kanaDist = kanaSimilarity.getKanaSimilarity(param);
		const words = getSimilarWord(wordlist, target, kanaDist, length, param.VARIATION_COST) || [];
		return words.slice(0, length).map(w=>Object.assign({}, w));
	}
	
	function generate(phrases, wordlist, parameter, updateFunc,endFunc){
		const tokens_list = textAnalyzer.tokenizeTogether(phrases);
		return generateFromTokens(tokens_list, wordlist, parameter, updateFunc, endFunc);
	}
	...
	function generateFromTokens(tokens_list, wordlist, parameter, updateFunc,endFunc, locksPerLine=null){
		const param = assignDefaultParameter(parameter);
		const gs = (function(){
			const kanaDist = kanaSimilarity.getKanaSimilarity(param);
			const gsmemo = {}
		...
		function convert_line(i,delayMsec){
			...
			if(i>=tokenized_phrases.length){
				...
				setTimeout(()=>{
					...
					const raw_result = convert(tokens, gs, used_words, param,
